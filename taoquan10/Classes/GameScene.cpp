...
//    this->addChild(down,1);
    ...
    for (int i=0; i<9; i++) {
        ...
        dizuoTouying->setPosition(Vec2(origin.x+visibleSize.width*pX*0.01, origin.y + visibleSize.height*pY*0.01-visibleSize.height*0.12));
...
//        dizuoVector.pushBack(dizuo);
    ...
    for (int i=0; i<hiddenPic.size(); i++) {
        ...
        if (_inttag>99) {
            ResolvePicture* rp=ResolvePicture::createResolvePicture();
            rp->setScale(_spriteQuan->getContentSize().width*10/visibleSize.width);
...
void GameScene::setNextLevelPicture(EventCustom* e)
{
    ...
    for (int i=0; i<spriteVector.size(); i++) {
        ...
        if (cLevel>9) {
            ...
            dzty->setPosition(Vec2(origin.x+visibleSize.width*pX*0.01, origin.y + visibleSize.height*pY*0.01-visibleSize.height*0.12));
            //更换背景图片
            char bgName[40];
