...
bool GameScene::init()
{
    ...
    for (int i=0; i<4; i++) {
        for (int j=0; j<3; j++) {
            ...
            log("%f,%f",origin.x+visibleSize.width*0.2+visibleSize.width*0.2*i, origin.y + visibleSize.height*8/10-visibleSize.height*0.13*j);
            this->addChild(rp, 2);
...
//            this->addChild(dizuo, 1);
    ...
    //添加大图
    Sprite* quantu1 = Sprite::create("gamescene/11.png");
