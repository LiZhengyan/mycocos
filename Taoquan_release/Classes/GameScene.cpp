...
void GameScene::onTouchEnded(Touch *touch, Event *event)
{
    ...
    if (_isTouch) {
        ...
        if (_spriteTiao->getRotation()>0) {
            ...
            _isTouch=false;
            MoveTo* quanMove=MoveTo::create(s/(visibleSize.width*1.4), Vec2(x, y));
...
void GameScene::update(float dt)
{
    ...
    if (_spriteQuan->getPositionX()==x&&_spriteQuan->getPositionY()==y) {
        ...
        for (int i=0; i<spriteVector.size(); i++) {
            ...
            if (sqrt((x-showRp->getPositionX())*(x-showRp->getPositionX())+(y-showRp->getPositionY())*(y-showRp->getPositionY()))<=33.0) {
