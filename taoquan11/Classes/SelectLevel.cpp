...
bool SelectLevel::init()
{
   if ( !Layer::init() )
    ...
    if (isPropExist==false) {
        ...
        //像表格中插入数据
        string insertProp = "insert into Prop values(1000,1000,10)";
...
//                        }
    ...
    LabelAtlas*sPropNumber=LabelAtlas::create(":10", "selectlevel/propNumber.png", 17.0f, 24.0f, '0');
    sPropNumber->setPosition(Vec2(visibleSize.width*0.25, visibleSize.height-transparent->getContentSize().height*0.7));
    sPropNumber->setScale(visibleSize.width/640);
    this->addChild(sPropNumber,2);
    char spropChar[20];
    sprintf(spropChar, ":%d",PropNumber1);
    sPropNumber->setString(spropChar);
    
    LabelAtlas*hPropNumber=LabelAtlas::create(":10", "selectlevel/propNumber.png", 17.0f, 24.0f, '0');
    hPropNumber->setPosition(Vec2(visibleSize.width*0.5, visibleSize.height-transparent->getContentSize().height*0.7));
    hPropNumber->setScale(visibleSize.width/640);
    this->addChild(hPropNumber,2);
    char hpropChar[20];
    sprintf(hpropChar, ":%d",PropNumber0);
    hPropNumber->setString(hpropChar);
    LabelAtlas*cPropNumber=LabelAtlas::create(":10", "selectlevel/propNumber.png", 17.0f, 24.0f, '0');
    cPropNumber->setPosition(Vec2(visibleSize.width*0.75, visibleSize.height-transparent->getContentSize().height*0.7));
    cPropNumber->setScale(visibleSize.width/640);
    this->addChild(cPropNumber,2);
    char cpropChar[20];
    sprintf(cpropChar, ":%d",PropNumber2);
    cPropNumber->setString(cpropChar);
